#!/usr/bin/env python3
"""Execute C005 from an immutable run-scoped source bundle."""
import hashlib, importlib.util, json, sys, zipfile
from pathlib import Path

WORK = Path('/kaggle/working')
REPO = WORK / 'zaskaleta-ai-twin-colab'
EXPECTED = 'MASTER_CLONE_CANDIDATE_005'
...
def main():
    ...
    if identity.get('candidate_id') != EXPECTED or not token:
        raise RuntimeError('invalid C005 run identity')
    if len(source_sha) != 40 or any(c not in '0123456789abcdef' for c in source_sha):
        raise RuntimeError('invalid source SHA')
    bundle = here / 'source_bundle.zip'
    if digest(bundle) != identity['bundle_sha256']:
        raise RuntimeError('C005 source bundle digest mismatch')
    if REPO.exists():
        raise RuntimeError('refusing to overwrite existing C005 workspace')
    ...
    for name, expected in identity['critical_source_hashes'].items():
        ...
        if not path.is_file() or digest(path) != expected:
            raise RuntimeError('pinned C005 source mismatch: ' + name)
    spec = importlib.util.spec_from_file_location('c005_kernel', REPO / 'kaggle' / 'autopilot_kernel_c005.py')
    ...
    if rc != 0:
        return rc
    status = json.loads((WORK / 'zaskaleta_c005_status.json').read_text(encoding='utf-8'))
    if status.get('candidate_id') != EXPECTED or status.get('state') != 'C005_RENDER_READY_FOR_MANUAL_REVIEW':
        raise RuntimeError('C005 terminal status mismatch')
    if status.get('render_completed') is not True or status.get('auto_promote') is not False or status.get('stable_release_modified') is not False:
        raise RuntimeError('C005 safety/terminal status mismatch')
    if status.get('subjective_identity_review') != 'PENDING_MANUAL_REVIEW':
        raise RuntimeError('C005 bypassed manual identity review')
    qa = status.get('stream_qa') or {}
    if float(qa.get('video_duration') or 0) < 7.5:
        raise RuntimeError('C005 output stream QA missing/invalid')
    print('VERIFIED_C005_RENDER_READY_FOR_MANUAL_REVIEW', flush=True)
    return 0
