...
REQUIRED = [
    ...
    'content/storage_config.json','content/clone_quality_gate_v1.json','content/clone_release_policy_v1.json','content/clone_duration_gate_policy_v1.json',
    'content/clone_promotion_bundle_policy_v1.json','content/clone_memory_policy_v1.json',
]
...
def main() -> int:
    ap = argparse.ArgumentParser(); ap.add_argument('--require-runtime-env', action='store_true'); args = ap.parse_args()
    failures: list[str] = []
    missing = [rel for rel in REQUIRED if not (ROOT / rel).is_file()]
    if missing: failures.extend('missing:' + rel for rel in missing)

    if not missing:
        cfg = json.loads(read('content/storage_config.json')); canonical = cfg.get('canonical_storage') or {}; runtime = cfg.get('runtime') or {}; encryption = cfg.get('encryption') or {}
        ...
        api = read('runpod/api_server.py'); runner = read('worker/run_clone_v2_test.py'); materializer = read('worker/materialize_clone_runtime_from_s3.py')
        artifact_store = read('worker/clone_job_artifact_store.py'); state_store = read('worker/clone_job_state_store.py'); provenance = read('worker/validate_lipsync_render_provenance.py')
        requirements = read('runpod/requirements-api.txt'); ignore = read('.gitignore'); promotion = json.loads(read('content/clone_promotion_bundle_policy_v1.json'))
        ...
        for token in ('zaskaleta-clone-job-state-manifest-v1','VERIFIED_DECRYPTED_SHA256',"sub.add_parser('persist')", "sub.add_parser('restore')",'strict_key'):
            if token not in state_store: failures.append('state_store_missing:' + token)
        for token in ('clone_job_artifact_store.py','clone_job_state_store.py','persist_job_state','restore_job_state','encrypted_job_state_recovery','artifacts_persisted_encrypted=True','cleanup_job_plaintext','BackgroundTask','JOB_ID_RE','ACTIVE_JOB_LOCK','reserve_render_slot','release_render_slot(job_id)',"status_code=429","'max_concurrent_render_jobs_per_worker': 1"):
        ...
        if args.require_runtime_env:
            ...
            for name in env_names:
                if not isinstance(name, str) or not name or not os.environ.get(name, '').strip(): failures.append('runtime_env_missing:' + str(name))

    report = {
        'schema': 'zaskaleta-clone-external-connection-readiness-v5',
        ...
        'encrypted_job_state_recovery_required': True,
        'max_concurrent_render_jobs_per_worker': 1,
        'external_connection_performed': False,'network_action_performed': False,'paid_gpu_started': False,
        'manual_budget_approval_required_before_paid_gpu': True,'secret_values_exposed': False,'first_gpu_gate_seconds': [8, 15],
        'failures': failures,
