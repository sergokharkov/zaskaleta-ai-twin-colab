...
VOICE_PRESETS = {
    'calm': {'tempo': 0.95, 'pause_scale': 1.05, 'compressor_ratio': 1.65, 'lra': 11},
    'confident': {'tempo': 0.98, 'pause_scale': 0.92, 'compressor_ratio': 1.8, 'lra': 10},
    'serious': {'tempo': 0.94, 'pause_scale': 1.12, 'compressor_ratio': 1.7, 'lra': 11},
    'warm': {'tempo': 0.96, 'pause_scale': 1.04, 'compressor_ratio': 1.6, 'lra': 12},
...
def pause_for(phrase: str, mode: str, preset: str):
    scale = VOICE_PRESETS[preset]['pause_scale']
    base = 0.10 if mode == 'talking' else 0.12
    if phrase.endswith('…'):
        value = 0.34
...
def apply_voice_dynamics(src: Path, dst: Path, mode: str, preset: str):
    cfg = VOICE_PRESETS[preset]
    tempo = cfg['tempo'] if mode == 'talking' else min(1.0, cfg['tempo'] + 0.02)
    ratio = cfg['compressor_ratio']
    lra = cfg['lra']
    threshold = -21 if mode == 'talking' else -19
    attack = 22 if mode == 'talking' else 16
    release = 190 if mode == 'talking' else 150
    af = (
        f'atempo={tempo},'
        'highpass=f=60,'
        f'acompressor=threshold={threshold}dB:ratio={ratio}:attack={attack}:release={release}:makeup=1.15,'
        f'loudnorm=I=-16:TP=-1.5:LRA={lra}'
    )
    subprocess.run([
...
def synth(text, tokenizer, tts, converter, target_se, work, out_path, device, mode='voiceover', preset='conversational'):
    ...
    for phrase in phrases:
        ...
        assembled.append(np.zeros(int(sample_rate * pause_for(clean, mode, preset)), dtype=np.float32))
    ...
    apply_voice_dynamics(converted, out_path, mode, preset)
...
def main():
    ...
    ap.add_argument('--voice-preset', choices=sorted(VOICE_PRESETS), default='conversational')
    args = ap.parse_args()
    ...
    for scene in scenes:
        ...
        synth(text, tokenizer, tts, converter, target_se, out, wav, device, mode=mode, preset=args.voice_preset)
