...
def split_phrases(text: str):
    # Keep punctuation attached so we can translate it into natural pauses.
    parts = re.findall(r'[^,;:—–.!?…]+[,;:—–.!?…]*', text.strip())
    return [p.strip() for p in parts if p.strip()]


def pause_for(phrase: str, mode: str):
    base = 0.16 if mode == 'talking' else 0.12
    if phrase.endswith('…'):
        return 0.50 if mode == 'talking' else 0.38
    if phrase.endswith(('!', '?')):
        return 0.34 if mode == 'talking' else 0.26
    if phrase.endswith(('.',)):
        return 0.30 if mode == 'talking' else 0.22
    if phrase.endswith(('—', '–')):
        return 0.28 if mode == 'talking' else 0.20
    if phrase.endswith((',', ';', ':')):
        return 0.20 if mode == 'talking' else 0.15
    return base
...
def apply_voice_dynamics(src: Path, dst: Path, mode: str):
    # Talking beats: deliberate, firm delivery. Voiceover: slightly quicker and smoother.
    tempo = 0.93 if mode == 'talking' else 0.98
    af = (
        f'atempo={tempo},'
        'highpass=f=70,'
        'acompressor=threshold=-18dB:ratio=2.2:attack=12:release=120:makeup=1.5,'
...
def synth(text, tokenizer, tts, converter, target_se, work, out_path, device, mode='voiceover'):
    # Generate phrase-by-phrase rather than one long monotone sentence.
    ...
    for phrase in phrases:
        ...
        # Trim excessive near-silence generated by VITS while retaining natural attack/release.
        if wav.size:
        ...
        pause = np.zeros(int(sample_rate * pause_for(clean, mode)), dtype=np.float32)
...
def main():
    ...
    print('✅ Expressive pacing: phrase pauses + motivational talking cadence + smoother voiceover')
